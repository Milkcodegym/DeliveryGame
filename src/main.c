...
/*
 * Description: Checks for the 'resources' directory and adjusts the working directory if necessary.
 * Parameters: None.
 * Returns: None.
 */
void FixPath(void) {
...
int main(void)
{
    ...
    InitWindow(1280, 720, "Delivery Game");

    int monitorWidth = GetMonitorWidth(0);
    int monitorHeight = GetMonitorHeight(0);
    float screen_percent = 0.8f;
    int screenWidth = (int)(screen_percent * monitorWidth);
    int screenHeight = (int)(screen_percent * monitorHeight);
    ...
    // Outer loop for Game/Menu resets
        ...
        while (!WindowShouldClose()) {
                    ...
                    if (player.fuel <= 0.0f) {
                        ...
                        if (IsKeyPressed(KEY_R)) {
                            ...
                            if (player.money < 25.0f) {
                                player.fuel += 10.0f; // Charity fuel
            ...
            if (GetDealershipState() == DEALERSHIP_ACTIVE) {
                ...
                if (borderMessageTimer > 0.0f) {
                    ...
                    // int screenH = GetScreenHeight(); // Unused variable
